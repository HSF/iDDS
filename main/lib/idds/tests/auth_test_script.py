...
class TestAuthentication:
    ...
    def test_oidc_authentication(self):
        ...
        print(
            (
                "Please go to {0} and sign in. "
                "Waiting until authentication is completed"
            ).format(sign_url["verification_uri_complete"])
        )
        ...
        while datetime.datetime.utcnow() - start_time < datetime.timedelta(
            seconds=expires_in
        ):
            try:
                ...
                if status:
                    ...
                    if (
                        type(output) in [dict]
                        and "error" in output
                        and output["error"] == "authorization_pending"
                    ):
                        time.sleep(interval)
                    else:
                        print(output)
        ...
        if not token:
            ...
            status, decoded_token, username = oidc.verify_id_token(
                vo, token["id_token"]
            )
            if not status:
