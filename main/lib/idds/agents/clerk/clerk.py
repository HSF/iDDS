...
from idds.agents.common.baseagent import BaseAgent, BaseAgentWorker
...
class ClerkWorker(BaseAgentWorker):
    """
    Clerk workers
    """
    ...
    def __init__(
            ...
            poll_period=10,
            new_poll_period=10,
            update_poll_period=10,
            throttle_poll_period=10,
            cache_expire_seconds=300,
            poll_period_increase_rate=2,
            max_new_poll_period=3600 * 6,
            max_update_poll_period=3600 * 6,
            pending_time=None,
            new_command_poll_period=1,
            update_command_poll_period=5,
            max_new_retries=3,
            max_update_retries=0):
        super(ClerkWorker, self).__init__()

        self.poll_period = int(poll_period)
        self.new_poll_period = int(new_poll_period)
        self.update_poll_period = int(update_poll_period)
        self.throttle_poll_period = int(throttle_poll_period)
        self.cache_expire_seconds = int(cache_expire_seconds)
        self.max_new_poll_period = int(max_new_poll_period)
        self.max_update_poll_period = int(max_update_poll_period)
        self.poll_period_increase_rate = float(poll_period_increase_rate)

        self.pending_time = pending_time
        self.new_command_poll_period = int(new_command_poll_period)
        self.update_command_poll_period = int(update_command_poll_period)
        self.max_new_retries = int(max_new_retries)
        self.max_update_retries = int(max_update_retries)
    ...
    def get_request(self, request_id, status=None, locking=False):
        try:
            ...
            if 'ORA-00060' in str(ex):
                logger.warn("(cx_Oracle.DatabaseError) ORA-00060: deadlock detected while waiting for resource")
            else:
                # raise ex
                logger.error(ex)
    ...
    def whether_to_throttle(self, request):
        ...
        try:
            ...
            logger.info("throttler(site: %s): active requests(%s), transforms(%s), processings(%s)" % (site, num_requests, num_transforms, num_processings))
            logger.info("throttler(site: %s): active input contents(%s), output contents(%s)" % (site, num_input_contents, num_output_contents))
            ...
            logger.info("throttler(site: %s): throttle_requests %s, throttle_transforms: %s, throttle_processings: %s" % (site, throttle_requests, throttle_transforms, throttle_processings))
            if throttle_requests:
                if num_requests['processing'] >= throttle_requests:
                    logger.info("throttler(site: %s): num of processing requests (%s) is bigger than throttle_requests (%s), set throttling" % (site, num_requests['processing'], throttle_requests))
                    return True
            if throttle_transforms:
                if num_transforms['processing'] >= throttle_transforms:
                    logger.info("throttler(site: %s): num of processing transforms (%s) is bigger than throttle_transforms (%s), set throttling" % (site, num_transforms['processing'], throttle_transforms))
                    return True
            if throttle_processings:
                if num_processings['processing'] >= throttle_processings:
                    logger.info("throttler(site: %s): num of processing processings (%s) is bigger than throttle_processings (%s), set throttling" % (site, num_processings['processing'], throttle_processings))
                    return True
            ...
            logger.info("throttler(site: %s): throttle_new_jobs: %s, throttle_queue_jobs: %s" % (site, throttle_new_jobs, throttle_queue_jobs))
            logger.info("throttler(site: %s): new_jobs: %s, queue_jobs: %s" % (site, new_jobs, queue_jobs))
            if throttle_new_jobs:
                if new_jobs >= throttle_new_jobs:
                    logger.info("throttler(site: %s): num of new jobs(not released) (%s) is bigger than throttle_new_jobs (%s), set throttling" % (site, new_jobs, throttle_new_jobs))
                    return True
            if throttle_queue_jobs:
                if queue_jobs >= throttle_queue_jobs:
                    logger.info("throttler(site: %s): num of queue jobs(released but not terminated) (%s) is bigger than throttle_queue_jobs (%s), set throttling" % (site, queue_jobs, throttle_queue_jobs))
                    return True

            return False
        except Exception as ex:
            logger.error("whether_to_throttle: %s" % str(ex))
            logger.error(traceback.format_exc())
        return False
    ...
    def handle_new_request(self, req):
        try:
            log_prefix = f"<request_id={req['request_id']}>"
            logger = self.get_logger(log_prefix)
            logger.info("Handle new request")
            ...
            if to_throttle:
                ...
                logger.info("Throttle new request result: %s" % str(ret_req))
            else:
                ...
                logger.debug("Processing request(%s): new transforms: %s" % (req['request_id'], str(transforms)))
                ...
                logger.info("Handle new request result: %s" % str(ret_req))
        except Exception as ex:
            logger.error(ex)
            logger.error(traceback.format_exc())
            ...
            logger.warn("Handle new request error result: %s" % str(ret_req))
        return ret_req
    ...
    def handle_new_irequest(self, req):
        try:
            log_prefix = f"<request_id={req['request_id']}>"
            logger = self.get_logger(log_prefix)
            logger.info("Handle new irequest")
            to_throttle = self.whether_to_throttle(req)
            if to_throttle:
                ret_req = {'request_id': req['request_id'],
                           'parameters': {'status': RequestStatus.Throttling,
                                          'locking': RequestLocking.Idle}}
                ret_req['parameters'] = self.load_poll_period(req, ret_req['parameters'], throttling=True)
                logger.info("Throttle new irequest result: %s" % str(ret_req))
            else:
                workflow = req['request_metadata']['workflow']
                ...
                ret_req = {'request_id': req['request_id'],
                           ...
                           'new_transforms': transforms}
                ret_req['parameters'] = self.load_poll_period(req, ret_req['parameters'])
                logger.info("Handle new irequest result: %s" % str(ret_req))
        except Exception as ex:
            logger.error(ex)
            logger.error(traceback.format_exc())
            retries = req['new_retries'] + 1
            ...
            ret_req = {'request_id': req['request_id'],
                       'parameters': {'status': req_status,
                                      ...
                                      'errors': req['errors'] if req['errors'] else {}}}
            ret_req['parameters']['errors'].update(error)
            logger.warn("Handle new irequest error result: %s" % str(ret_req))
        return ret_req
    ...
    def handle_new_generic_request(self, req):
        try:
            log_prefix = f"<request_id={req['request_id']}>"
            logger = self.get_logger(log_prefix)
            logger.info("Handle new generic request")
            to_throttle = self.whether_to_throttle(req)
            if to_throttle:
                ret_req = {'request_id': req['request_id'],
                           'parameters': {'status': RequestStatus.Throttling,
                                          'locking': RequestLocking.Idle}}
                ret_req['parameters'] = self.load_poll_period(req, ret_req['parameters'], throttling=True)
                logger.info("Throttle new generic request result: %s" % str(ret_req))
            else:
                workflow = req['request_metadata']['workflow']
                ...
                for w in works:
                    ...
                    if transform:
                        transforms.append(transform)
                logger.debug(f"Processing request({req['request_id']}): new transforms: {transforms}")
                ...
                logger.debug(f"Processing request({req['request_id']}), new conditions: {conditions}")
                ...
                ret_req['parameters'] = self.load_poll_period(req, ret_req['parameters'])
                logger.info("Handle new generic request result: %s" % str(ret_req))
        except Exception as ex:
            logger.error(ex)
            logger.error(traceback.format_exc())
            retries = req['new_retries'] + 1
    ...
    def has_to_build_work(self, req):
        try:
            if req['status'] in [RequestStatus.New] and 'build_workflow' in req['request_metadata']:
                log_prefix = self.get_log_prefix(req)
                logger = self.get_logger(log_prefix)
                logger.info("has build work")
                return True
    ...
    def handle_build_request(self, req):
        try:
            log_prefix = self.get_log_prefix(req)
            logger = self.get_logger(log_prefix)
            logger.info("handle build request")
            ...
            for work in works:
                ...
                transforms.append(transform)
            logger.debug("Processing request(%s): new build transforms: %s" % (req['request_id'], str(transforms)))
            ...
            ret_req = {'request_id': req['request_id'],
                       ...
                       'new_transforms': transforms}
            ret_req['parameters'] = self.load_poll_period(req, ret_req['parameters'])
            logger.info("Handle build request result: %s" % str(ret_req))
        except Exception as ex:
            logger.error(ex)
            logger.error(traceback.format_exc())
            retries = req['new_retries'] + 1
            ...
            ret_req = {'request_id': req['request_id'],
                       'parameters': {'status': req_status,
                                      ...
                                      'errors': req['errors'] if req['errors'] else {}}}
            ret_req['parameters']['errors'].update(error)
            logger.warn("Handle build request error result: %s" % str(ret_req))
        return ret_req
    ...
    def update_request(self, req, origin_req=None):
        ...
        try:
            log_prefix = self.get_log_prefix(req)
            logger = self.get_logger(log_prefix)
            logger.info("update request: %s" % req)
            ...
            try:
                ...
                logger.warn("Update request in exception: %s" % str(req_parameters))
    ...
    def process_new_request(self, event=None, request=None):
        try:
            if request:
                ...
                log_prefix = f"<request_id={event._request_id}>"
            else:
                log_prefix = None
            logger = self.get_logger(log_prefix)

            if request is None and event:
                ...
                if not req:
                    logger.error("Cannot find request for event: %s" % str(event))
                request = req

            if request:
                req = request
                logger.info(f"process_new_request request: {req['request_id']} event: {event}")
                ...
                for tf_id in new_tf_ids:
                    ...
                    logger.info("QueueTransformEvent(transform_id: %s)" % str(tf_id))
                    event = QueueTransformEvent(publisher_id=self.id, transform_id=tf_id)
                    self.event_bus.send(event)
                    # time.sleep(1)
                for tf_id in update_tf_ids:
                    logger.info("UpdateTransformEvent(transform_id: %s)" % str(tf_id))
    ...
    def handle_update_request_real(self, req, event):
        ...
        logger.info(" handle_update_request: request_id: %s" % req['request_id'])
        ...
        if (event and event._content and 'cmd_type' in event._content and event._content['cmd_type']
            ...
            to_abort = True
            logger.info("to_abort: %s" % to_abort)
            if (event and event._content and 'cmd_content' in event._content and event._content['cmd_content']
                ...
                logger.info("to_abort_transform_id: %s" % to_abort_transform_id)
        ...
        for work in works:
            ...
            if work.get_work_id():
                ...
                if tf:
                    ...
                    logger.info("transform status: %s, work status: %s" % (tf['status'], work.status))
            ...
            if not found_match_works:
                ...
                if not tfs:
                    logger.info(f"Found transforms with request_id {req['request_id']} internal_ids [{work.internal_id}]: {tfs}")
                else:
                    tf_ids = [tf['transform_id'] for tf in tfs]
                    logger.info(f"Found transforms with request_id {req['request_id']} internal_ids [{work.internal_id}]: {tf_ids}")
                if len(tfs) == 1:
                    tf = tfs[0]
                    all_released_work_status.append(tf['status'])
                    transform_work = tf['transform_metadata']['work']
                    if transform_work.internal_id == work.internal_id:
                        if hasattr(work, 'set_work_id'):
                            work.set_work_id(tf['transform_id'], transforming=True)
                        work.sync_work_data(status=tf['status'], substatus=tf['substatus'], work=transform_work, workload_id=tf['workload_id'])
                        logger.info("transform status: %s, work status: %s" % (tf['status'], work.status))
                    else:
        ...
        new_transforms = []
        logger.info(f"request status: {req['status']} and to_cancel: {wf.to_cancel}")
        ...
        if True:
            ...
            for work in works:
                tfs = core_transforms.get_transforms(request_id=req['request_id'], internal_ids=work.internal_id, loop_index=work.get_loop_index())
                if not tfs:
                    logger.info(f"Found transforms for new work with request_id {req['request_id']} internal_ids [{work.internal_id}]: {tfs}")
                else:
                    tf_ids = [tf['transform_id'] for tf in tfs]
                    logger.info(f"Found transforms for new work with request_id {req['request_id']} internal_ids [{work.internal_id}]: {tf_ids}")
                if len(tfs) == 1:
                    tf = tfs[0]
                    transform_work = tf['transform_metadata']['work']
                    if transform_work.internal_id == work.internal_id:
            ...
            logger.debug("Processing request(%s): new transforms: %s" % (req['request_id'], str(new_transforms)))
        ...
        if req_status in [RequestStatus.Transforming]:
            if wf.is_to_expire(req['expired_at'], self.pending_time, request_id=req['request_id']):
                ...
                event_content = {'request_id': req['request_id'],
                                 ...
                                 'cmd_content': {}}
                logger.debug("ExpireRequestEvent(request_id: %s)" % req['request_id'])
        ...
        logger.info("Handle update request result: %s" % str(ret))
        return ret
    ...
    def handle_update_build_request_real(self, req, event):
        ...
        log_prefix = self.get_log_prefix(req)
        logger = self.get_logger(log_prefix)
        logger.info("handle_update_build_request: request_id: %s" % req['request_id'])
        wf = req['request_metadata']['build_workflow']
        ...
        if req['status'] in [RequestStatus.Building] and not wf.to_cancel:
            ...
            for work in works:
                ...
                new_transforms.append(new_transform)
            logger.debug(" Processing build request(%s): new transforms: %s" % (req['request_id'], str(new_transforms)))
        ...
        try:
            ...
            ret_req = {'request_id': req['request_id'],
                       'parameters': {'status': req_status,
                                      ...
                                      'errors': req['errors'] if req['errors'] else {}}}
            ret_req['parameters']['errors'].update(error)
            logger.warn("Handle update request exception result: %s" % str(ret_req))
        return ret_req
    ...
    def is_to_expire(self, expired_at=None, pending_time=None, request_id=None):
        if expired_at:
            ...
            if expired_at < datetime.datetime.utcnow():
                ...
                logger.info("Request(%s) expired_at(%s) is smaller than utc now(%s), expiring" % (request_id,
                                                                                                  expired_at,
    ...
    def handle_update_irequest_real(self, req, event):
        ...
        log_prefix = self.get_log_prefix(req)
        logger = self.get_logger(log_prefix)
        logger.info(" handle_update_irequest: request_id: %s" % req['request_id'])
        ...
        log_msg = "ireqeust %s status: %s" % (req['request_id'], req_status)
        ...
        ret = {'request_id': req['request_id'],
               'parameters': parameters}
        logger.info("Handle update irequest result: %s" % str(ret))
        return ret
        ...
        try:
            ...
            ret_req = {'request_id': req['request_id'],
                       'parameters': {'status': req_status,
                                      ...
                                      'errors': req['errors'] if req['errors'] else {}}}
            ret_req['parameters']['errors'].update(error)
            logger.warn("Handle update irequest exception result: %s" % str(ret_req))
        return ret_req
    ...
    def process_update_request(self, event=None, request=None):
        try:
            ...
            if request is None and event:
                ...
                # req = self.get_request(request_id=event._request_id, status=req_status, locking=True)
                logger.debug("process_update_request: event: %s" % str(event))
            ...
            if request:
                req = request
                logger.info(f"process_update_request request: {req['request_id']} event: {event}")
                ...
                for tf_id in new_tf_ids:
                    ...
                    self.event_bus.send(event)
                for tf_id in update_tf_ids:
                    logger.info("UpdateTransformEvent(transform_id: %s)" % tf_id)
    ...
    def handle_abort_request(self, req, event):
        ...
        try:
            log_prefix = self.get_log_prefix(req)
            logger = self.get_logger(log_prefix)
            logger.info("handle_abort_request event: %s" % str(event))
            ...
            logger.info("handle_abort_request result: %s" % str(ret_req))
            return ret_req
        except Exception as ex:
            logger.error(ex)
            logger.error(traceback.format_exc())
            ...
            ret_req = {'request_id': req['request_id'],
                       'parameters': {'status': RequestStatus.ToCancel,
                                      ...
                                      'errors': req['errors'] if req['errors'] else {}}}
            ret_req['parameters']['errors'].update(error)
            logger.info("handle_abort_request exception result: %s" % str(ret_req))
        return ret_req
    ...
    def process_abort_request(self, event=None, request=None, command=None):
        try:
            ...
            if request:
                req = request
                logger.info(f"process_abort_request request: {req['request_id']}, event: {event}, command: {command}")
                ...
                if req['status'] in [RequestStatus.Finished, RequestStatus.SubFinished,
                    ...
                    logger.info("process_abort_request result: %s" % str(ret))
                    ...
                    core_transforms.abort_resume_transforms(request_id=req['request_id'], abort=True)
                    ...
                    if works:
                        ...
                        for work in works:
                            if (work.is_started() or work.is_starting()) and not work.is_terminated():
                                if not to_abort_transform_id or to_abort_transform_id == work.get_work_id():
                                    logger.info("AbortTransformEvent(transform_id: %s)" % str(work.get_work_id()))
                        ...
                        if not has_abort_work:
                            logger.info("not has abort work")
                            logger.info("UpdateRequestEvent(request_id: %s)" % str(req['request_id']))
    ...
    def handle_close_irequest(self, req, event):
        ...
        try:
            log_prefix = self.get_log_prefix(req)
            logger = self.get_logger(log_prefix)
            logger.info("handle_close_irequest event: %s" % str(event))
            ...
            ret = {'request_id': req['request_id'],
                   'parameters': parameters}
            logger.info("Handle close irequest result: %s" % str(ret))
            return ret
            ...
            ret_req = {'request_id': req['request_id'],
                       'parameters': {'status': RequestStatus.ToClose,
                                      ...
                                      'errors': req['errors'] if req['errors'] else {}}}
            ret_req['parameters']['errors'].update(error)
            logger.info("handle_close_irequest exception result: %s" % str(ret_req))
        return ret_req
    ...
    def process_close_request(self, event=None, request=None, command=None):
        try:
            ...
            if request:
                req = request
                logger.info(f"process_close_request request: {req['request_id']} event: {event}")
    ...
    def handle_resume_irequest(self, req, event):
        ...
        try:
            log_prefix = self.get_log_prefix(req)
            logger = self.get_logger(log_prefix)
            logger.info("handle_resume_irequest event: %s" % str(event))
            ...
            ret = {'request_id': req['request_id'],
                   'parameters': parameters}
            logger.info("Handle resume irequest result: %s" % str(ret))
            return ret
        except Exception as ex:
            logger.error(ex)
            logger.error(traceback.format_exc())
    ...
    def process_resume_request(self, event=None, request=None, command=None):
        try:
            ...
            if request:
                req = request
                logger.info(f"process_resume_request request: {req['request_id']} event: {event}")
                ...
                if req['status'] in [RequestStatus.Finished]:
                    ...
                    if req['errors'] and 'msg' in req['errors']:
                        ret['parameters']['errors']['msg'] = req['errors']['msg']
                    logger.info("process_resume_request result: %s" % str(ret))
                    ...
                    if 'workflow' in req['request_metadata']:
                        ...
                        if works:
                            for work in works:
                                ...
                                logger.info("ResumeTransformEvent(transform_id: %s)" % str(work.get_work_id()))
                                event = ResumeTransformEvent(publisher_id=self.id,
                                                             transform_id=work.get_work_id(),
                                                             content=event._content if event else None)
                                self.event_bus.send(event)
...
class Clerk(BaseAgent):
    ...
    def __init__(self, num_threads=1, max_number_workers=8, poll_period=10, retrieve_bulk_size=3, cache_expire_seconds=300, pending_time=None, **kwargs):
        ...
        if not hasattr(self, 'throttle_poll_period') or not self.throttle_poll_period:
            ...
            self.throttle_poll_period = int(self.throttle_poll_period)
        ...
        self.worker = ClerkWorker(
            poll_period=self.poll_period,
            new_poll_period=self.new_poll_period,
            update_poll_period=self.update_poll_period,
            throttle_poll_period=self.throttle_poll_period,
            cache_expire_seconds=self.cache_expire_seconds,
            poll_period_increase_rate=self.poll_period_increase_rate,
            max_new_poll_period=self.max_new_poll_period,
            max_update_poll_period=self.max_update_poll_period,
            pending_time=self.pending_time,
            new_command_poll_period=self.new_command_poll_period,
            update_command_poll_period=self.update_command_poll_period,
            max_new_retries=self.max_new_retries,
            max_update_retries=self.max_update_retries
        )
    ...
    def get_new_requests(self):
        ...
        try:
            ...
            reqs_new = core_requests.get_requests_by_status_type(status=req_status, locking=True,
                                                                 ...
                                                                 new_poll=True, only_return_id=False)

            # self.logger.debug("Main thread get %s [New+Extend] requests to process" % len(reqs_new))
            if reqs_new:
                req_ids = [req["request_id"] for req in reqs_new]
                self.logger.info("Main thread get [New+Extend] requests to process: %s" % str(req_ids))

            for req in reqs_new:
                req_id = req["request_id"]
                self.submit(self.worker.process_new_request, **{"request": req})
    ...
    def get_running_requests(self):
        ...
        try:
            ...
            reqs = core_requests.get_requests_by_status_type(status=req_status, time_period=None,
                                                             ...
                                                             locking=True,
                                                             bulk_size=self.get_bulk_size(),
                                                             update_poll=True, only_return_id=False)

            # self.logger.debug("Main thread get %s Transforming requests to running" % len(reqs))
            if reqs:
                req_ids = [req["request_id"] for req in reqs]
                self.logger.info("Main thread get Transforming requests to running: %s" % str(req_ids))

            for req in reqs:
                req_id = req["request_id"]
                self.submit(self.worker.process_update_request, **{"request": req})
                ...
                if cmd_status in [CommandStatus.New, CommandStatus.Processing]:
                    ...
                    if req:
                        if cmd_type in [CommandType.AbortRequest]:
                            self.submit(self.worker.process_abort_request, **{"request": req, "command": cmd["cmd_id"]})
                        elif cmd_type in [CommandType.ResumeRequest]:
                            self.submit(self.worker.process_resume_request, **{"request": req, "command": cmd["cmd_id"]})
                        elif cmd_type in [CommandType.CloseRequest]:
                            self.submit(self.worker.process_close_request, **{"request": req, "command": cmd["cmd_id"]})
    ...
    def clean_locks(self, force=False):
        try:
            ...
            core_requests.clean_locking(health_items=health_items, min_request_id=min_request_id,
