                ...
                try:
                    ...
                    if self.coordinator:
                        if req['type'] == 'send_event':
                            ...
                            wait = req['wait']
                            ret = self.coordinator.get(event_type, wait)
        ...
        with self._lock:
            ...
            if self._events_history_clean_time + 3600 * 4 < time.time():
                ...
                for event_type in self._events_index:
                    event_act_ids = []
                    for event_id in self._events_index[event._event_type]:
                        event = self._events[event._event_type][event_id]
                        act_id = event.get_event_id()
                        event_act_ids.append(act_id)

                    event_history_keys = list(self._events_history[event._event_type].keys())
                    for key in event_history_keys:
                        if key not in event_act_ids:
                            del self._events_history[event._event_type][key]

                    act_id_keys = list(self._events_act_id_index[event._event_type].keys())
                    for act_id_key in act_id_keys:
                        act_id2ids = self._events_act_id_index[event._event_type][act_id_key].copy()
                        for q_id in act_id2ids:
                            if q_id not in self._events_index[event._event_type]:
                                self._events_act_id_index[event._event_type][act_id_key].remove(q_id)
                        if not self._events_act_id_index[event._event_type][act_id_key]:
                            del self._events_act_id_index[event._event_type][act_id_key]
...
class MsgEventBusBackend(BaseEventBusBackend):
    ...
    def get(self, event_type, wait=0):
        with self._lock:
            try:
                req = {'type': 'get_event', 'event_type': event_type, 'wait': wait}
                req = json_dumps(req)
