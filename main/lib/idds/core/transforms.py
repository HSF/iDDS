...
@read_session
def get_transform_input_output_maps(request_id, transform_id, input_coll_ids, output_coll_ids, log_coll_ids=[], with_sub_map_id=False, is_es=False, with_deps=True, page_num=None, page_size=None, status=None, session=None):
    """
    ...
    """
    contents = orm_contents.get_contents_by_request_transform(request_id=request_id, transform_id=transform_id, with_deps=with_deps, page_num=page_num, page_size=page_size, status=status, by_map=True, session=session)
    ret = {}
