...
class HPOClient(BaseRestClient):
    ...
    def update_hyperparameter(self, request_id, id, loss):
        """
        ...
        url = self.build_url(self.host, path=os.path.join(path, str(request_id), str(id), str(loss)))

        r = self.get_request_response(url, type='PUT')
        return r

    def get_hyperparameters(self, request_id, status=None, limit=None):
        """
        ...
        if limit is None:
            limit = 'null'
        url = self.build_url(self.host, path=os.path.join(path, str(request_id), str(status), str(limit)))
